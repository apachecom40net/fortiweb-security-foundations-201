...
step_packages() {
  apt-get install -y \
    docker.io curl git jq netcat-openbsd dbus-x11 \
...
export XDG_RUNTIME_DIR=/tmp/.xrdp_${USER}
...
STARTWM
  ...
  echo "exec dbus-launch --exit-with-session startxfce4" > "/home/${ADMIN_USER}/.xsession"
